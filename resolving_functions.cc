...
inline void process_tainted_and_resolved_branch(ADDRINT ins_addr, bool br_taken, ptr_branch& tainted_ptr_branch)
{
  if (tainted_ptr_branch->br_taken != br_taken) // new branch taken
  {
    ...
    if (active_ptr_branch->checkpoint->rb_times <= max_local_rollback_times /*max_local_rollback.Value()*/)
    {
...
{
  std::cout << explored_trace.size() << " " << remove_leading_zeros(StringFromAddrint(ins_addr)) << "\n";
